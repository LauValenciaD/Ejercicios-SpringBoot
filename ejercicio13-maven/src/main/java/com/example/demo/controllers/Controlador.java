...
public class Controlador {
	...
	public ResponseEntity<List> obtenerTareasProximas(@PathVariable("dias") int dias) {
		...
		for (Tarea tarea : listaTareas) {
			if (tarea.getFechaFin().isBefore(fechaLimite) && tarea.getEstado() != Estado.COMPLETA) { // que este dentro
																										// de la fecha y
																										// no este
																										// completa
	...
	public ResponseEntity<List> obtenerTareasClave(@PathVariable("palabraClave") String palabraClave) {
		List<Tarea> listaClave = new ArrayList<>();
		for (Tarea tarea : listaTareas) {
			if (tarea.getDescripcion().toUpperCase().contains(palabraClave.toUpperCase())) {
				listaClave.add(tarea);
			}
		}
		if (listaClave.isEmpty()) {
			return ResponseEntity.notFound().build();
		} else {
			return ResponseEntity.ok(listaClave);
		}

	}
	/*
	 * Marcar tareas como completadas: PATCH /marcar-completadas: Todas las tareas
	 * vencidas, aquellas cuya fecha ya ha pasado, deben marcarse como COMPLETA
	 */

	@PatchMapping("/marcar-completadas")
	public ResponseEntity<Tarea> marcarCompletadas()
	{
		for (Tarea tarea : listaTareas) {
			if (tarea.getFechaFin().isBefore(LocalDate.now())) {
